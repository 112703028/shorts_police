...
def test_format_reply_trash():
    from line_bot import format_reply
    result = format_reply(overall_score=15, verdict="trash", summary="AI生成動物影片，無資訊價值")
    assert "❌" in result
...
def test_format_reply_keep():
    from line_bot import format_reply
    result = format_reply(overall_score=85, verdict="keep", summary="製作精良，資訊豐富")
    assert "✅" in result
    assert "好片" in result


def test_format_reply_review_fallback_for_unknown_verdict():
    from line_bot import format_reply
    result = format_reply(overall_score=50, verdict="unexpected", summary="邊界案例")
    assert "⚠️" in result
    assert "普通" in result
