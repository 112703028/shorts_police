...
ONBOARDING_QUESTIONS = """（使用者可以直接打字描述自己覺得什麼算廢片，也可以偷懶回數字參考選項：
1 = 心靈雞湯/勵志語錄，2 = 開箱業配，3 = AI生成動物/寵物，4 = 標題黨/誇張縮圖，0 = 都沒有）"""

ONBOARDING_CONTEXT = """【情境：首次使用問卷】
問的問題是：「你覺得什麼樣的 YouTube Shorts 算是廢片、不值得看？」
{questions}

使用者原始回覆：「{message}」
優先使用使用者自己的文字描述寫進品味檔案，盡量保留他原本的具體說法，不要硬套成罐頭分類；
只有在使用者回覆的是數字（例如「1,3」）時，才對照上面的選項轉換成類別名稱
（例如回覆「1,3」要寫成「討厭：心靈雞湯/勵志語錄、AI生成動物/寵物」，不要直接寫「類別1」「類別3」這種沒有意義的字）。"""
